...
bool StreamWav_Duration(WA_Input* pHandle, uint64_t* uBytesDuration);
bool StreamWav_Read(WA_Input* pHandle, int8_t* pByteBuffer, uint32_t uBytesToRead, uint32_t* uByteReaded);
bool StreamWav_GetWaveFormat(WA_Input* pHandle, uint32_t* uSamplerate, uint16_t* uChannels, uint16_t* uBitsPerSample);
bool StreamWav_IsStreamSeekable(WA_Input* pHandle);

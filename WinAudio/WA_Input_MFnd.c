...
bool MediaFoundation_Duration(WA_Input* pHandle, uint64_t* uBytesDuration);
bool MediaFoundation_Read(WA_Input* pHandle, int8_t* pByteBuffer, uint32_t uBytesToRead, uint32_t* uByteReaded);
bool MediaFoundation_GetWaveFormat(WA_Input* pHandle, uint32_t* uSamplerate, uint16_t* uChannels, uint16_t* uBitsPerSample);
bool MediaFoundation_IsStreamSeekable(WA_Input* pHandle);
...
static bool MediaFoundation_ReadSample(WA_Input* pHandle,
	int8_t** pByteBuffer,
	uint32_t* uBufferSize)
{
		...
		(pdwStreamFlags == MF_SOURCE_READERF_ENDOFSTREAM))
	{
		...
		if (pdwStreamFlags == MF_SOURCE_READERF_ENDOFSTREAM)
		{
			// Force this condition when end of stream is reached			
			MFInstance->uPosition = MFInstance->uDuration;
		}
